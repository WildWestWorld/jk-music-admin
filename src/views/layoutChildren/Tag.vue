<template>
  <div class="page">
    ...
    <div class="q-mt-md q-mb-md">
      <q-btn color="primary" label="添加标签" @click="toggleDialog()"/>
    </div>
...
<!--                </q-item>-->
    ...
    <div class="row justify-center q-mt-md">
      <q-pagination
          ...
          @click="updateData"
      />
      <CreateTagDialog ref="RefChildren" @fetchData="fetchData" :rowData="rowData">></CreateTagDialog>
      ...
      <q-dialog v-model="confirmDelete" persistent>
        <q-card>
          ...
          <q-card-actions align="right">
            ...
            <q-btn flat label="确定" color="primary" v-close-popup @click="deleteTagById()"/>
          </q-card-actions>
        </q-card>
      </q-dialog>
...
const columns = [
  ...
  {name:"name",label: '标签名', field: 'name', sortable: true, align: 'left'},
...
const getPageByName =(pageNum,pageSize,SearchWord)=>{
  ...
  getPageByTagName(pageNum, pageSize, SearchWord).then(res=>{
    console.log(pagination.value.rowsPerPage)
    console.log(res);
    rows.value = res.data.records;
    total.value = res.data.total;
...
const deleteTagById =()=>{
  if (rowId.value !== null) {
    deleteTag(rowId.value).then(res => {
      console.log(res)
      fetchData();
